...
def test_to_calendar_time() -> None:
    epoch = np.datetime64("2025-05-16T08:38:12.123456789", "ns")

    res = orbit.to_calendar_time(xr.DataArray(0.0), epoch)
    ...
    res = orbit.to_calendar_time(xr.DataArray(1e-10), epoch)
...
def test_OrbitPolyfitInterpolator_values(orbit_ds: xr.Dataset) -> None:
    ...
    distance = np.linalg.norm(
        orbit_interpolator.position(position.azimuth_time), axis=1
    )

    assert np.allclose(distance, 7_068_663, rtol=0.001)

    velocity = np.linalg.norm(
        orbit_interpolator.velocity(position.azimuth_time), axis=1
    )

    assert np.allclose(velocity, 7_590, rtol=0.001)

    acceleration = np.linalg.norm(
        orbit_interpolator.acceleration(position.azimuth_time), axis=1
    )
